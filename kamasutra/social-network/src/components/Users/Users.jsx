...
const Users = ({callFollow, callUnfollow, callSetUsers, users }) => {
  ...
  return (
    ...
    {users.map(({id, name, status, country, city, followed, pic}) => <User key={`${id}-${status}`} userId={id} name={name} status={status}
      country={country} city={city} followed={followed} pic={pic} handleFollow={handleFollow} handleUnfollow={handleUnfollow} /> )}
